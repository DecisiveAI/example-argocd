...
LOGIN_LOG=$(mktemp)
set +e
# The local Argo CD server on this cluster is serving HTTPS and works
# reliably with gRPC-web over the CLI's built-in port-forward.
argocd login "argocd-server" \
  ...
  --username admin \
  --password "$ADMIN_PASSWORD" >"$LOGIN_LOG" 2>&1
LOGIN_EXIT_CODE=$?
set -e
